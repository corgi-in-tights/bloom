...
async def check_time_restriction(user_id, current_date):
    async with get_async_session() as session:
        result = (
            await session.query(MathMatizeRestrictions)
            .filter(MathMatizeRestrictions.user_id == user_id)
            .limit(1)
            .first()
        )

        # check if it should be refreshed
        if result is not None and current_date >= result.release_date:
...
async def get_time_remaining(user_id, start_time, default_time, refresh_hours):
    async with get_async_session() as session:
        ...
        if result is None:
            set_time_restriction(user_id, start_time, default_time, refresh_hours)
...
async def reset_time_restriction(session, user_id):
    await (
        session.query(MathMatizeRestrictions)
        .filter(MathMatizeRestrictions.user_id == user_id)
